...
for(k in 1:3){
  for(v in 1:3){	
    ...
    E1<-rep(sort(c(10:28,29:41,29:41,42:56)),each=n.gran)
    
    E<-E1
    
    E_t = rep(sort(c(10:28,29:41,29:41,42:56)),each=200)
    ...
    for(i in 1:B){
      ...
      if(k==1){
        ...
        L1_16_pos<-rgamma(n.gran*n.each,l.param2,1/l.param2)
      ...
      if(k==2){
        #25
        l.param<-log(E1,base=1.5)-2.1
        l.param.inv<-(sum(range(l.param))+0-l.param)
        l.param2<-(sum(range(l.param))+0-l.param.inv)-(mean(l.param)-mean(l.param.inv))
        L<-rgamma(n.gran*n.each,l.param.inv,1/l.param.inv)
        
        L1_25_pos<-rgamma(n.gran*n.each,l.param2,1/l.param2)
        L_t = rgamma(10000,l.param2,1/l.param2)
      ...
      if(k==3){
        #36
        l.param<-log(E1,base=1.5)-1.1
        l.param.inv<-(sum(range(l.param))+0-l.param)
        l.param2<-(sum(range(l.param))+0-l.param.inv)-(mean(l.param)-mean(l.param.inv))
        
        L<-rgamma(n.gran*n.each,l.param.inv,1/l.param.inv)
        
        L1_36_pos<-rgamma(n.gran*n.each,l.param2,1/l.param2)
        L_t = rgamma(10000,l.param2,1/l.param2)
