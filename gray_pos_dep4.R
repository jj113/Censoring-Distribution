...
for(k in 1:3){
  for(v in 1:3){	
    ...
    E<-rep(sort(c(5:20,35:50,21:34,21:34)),each=n.gran)
    
    E<-E
    
    E_t<-rep(sort(c(5:20,35:50,21:34,21:34)),each=200)
    ...
    for(i in 1:B){
      if(k==1){
        # 16
        l.param<-log(E,base=1.5)-1.8
        l.param.inv<-(sum(range(l.param))+0.0-l.param)
        l.param2<-(sum(range(l.param))+0-l.param.inv)-(mean(l.param)-mean(l.param.inv))
        
        # special case because going below 0
        l.param2<-l.param2-min(0,range(l.param2)[1])+0.05
        
        L_16<-rgamma(n.gran*n.each,l.param.inv,1/l.param.inv)
        L<-rgamma(n.gran*n.each,l.param2,1/l.param2)
        
        L_t<-rgamma(10000,l.param2,1/l.param2)
        
        # change up the 'L' b/w negative and positive correlation in other scripts
      ...
      if(k==2){
        ...
        #25
        l.param<-log(E,base=1.5)-0.8
        l.param.inv<-(sum(range(l.param))+0-l.param)
        l.param2<-(sum(range(l.param))+0-l.param.inv)-(mean(l.param)-mean(l.param.inv))
        L_25<-rgamma(n.gran*n.each,l.param.inv,1/l.param.inv)
      ...
      if(k==3){
        #36
        l.param<-log(E,base=1.5)+0.2
        l.param.inv<-(sum(range(l.param))+0-l.param)
        l.param2<-(sum(range(l.param))+0-l.param.inv)-(mean(l.param)-mean(l.param.inv))
        
        L_36<-rgamma(n.gran*n.each,l.param.inv,1/l.param.inv)
    ...
    p3<-ggplot(data=data.frame())+geom_line(data=d.var,aes(x=times.ev,y=quo),colour="#555555",size=1.8,linetype=1)+
     ...
     # geom_line(data=d.surv,aes(x=time,y=surv),colour="#000000",size=1.8,linetype=3)+
